...
export class AutomationEngine {
  ...
  private async autoGenerateProposal(
    ...
    try {
      ...
      // Convert projectData to expected format
      const projectForProposal = {
        id: project.id,
        title: projectData.title,
        description: projectData.description,
        platform: projectData.platform,
        ...
        skills: projectData.skills,
        clientName: projectData.clientName,
        clientRating: projectData.clientRating?.toString() || null,
        projectUrl: projectData.projectUrl || projectData.url,
        deadline: projectData.deadline,
        status: project.status || 'open',
        matchScore: projectData.matchScore || 0,
        competitionLevel: projectData.competitionLevel || 'medium',
        successPrediction: projectData.successPrediction || 0,
        recommendedBid: projectData.recommendedBid || null,
        clientInsights: projectData.clientInsights || null,
        marketData: projectData.marketData || null,
        createdAt: project.createdAt,
        updatedAt: project.updatedAt
      };

      // Generate proposal using AI
      const proposalContent = await generateProposal(projectForProposal, profile);
      ...
      const [proposal] = await db
        ...
        .values({
          ...
          content: typeof proposalContent === 'string' ? proposalContent : proposalContent.content,
